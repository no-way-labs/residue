...
In the postscript to the revised paper (March~4, further revised March~6), Knuth reported a rapid sequence of developments: Stappers put Claude Opus~4.6 to work on the even case, producing a search-based solution via ORTools CP-SAT~\cite{knuth-even-solution}; Ho Boon Suan produced a computational construction for even $m \geq 8$ using GPT-5.3-codex~\cite{knuth-even-closed}, describing the pattern as ``far more chaotic'' than the odd case; Kim Morrison formalized Knuth's odd-case proof in Lean~4~\cite{morrison-lean}; and Maximilian Reitbauer found what Knuth called ``probably the simplest possible'' odd-case construction by interacting with GPT~5.4 and Claude~4.6 Sonnet~\cite{reitbauer2026}. Ho subsequently used GPT-5.4~Pro to produce a complete 14-page proof of~\cite{knuth-even-closed}'s correctness without human editing~\cite{ho-proof}. Finally, Knuth acknowledged the present work, describing the even-case construction as ``considerably simpler'' than~\cite{knuth-even-closed} and noting that the process analysis has ``potentially significant implications for how new problems can be tackled and resolved in the future.''
...
\begin{enumerate}[label=(\roman*)]
    ...
    \item \textbf{Methodological.} We document how these constructions were produced by two LLM agents operating under a shared structured exploration prompt, with an orchestrator agent (a third Claude Opus~4.6 Thinking instance) mediating between them. The same prompt applied to GPT-5.4 Thinking and Claude Opus~4.6 Thinking produced radically different mathematical strategies. The full exploration logs are included as appendices.
\end{enumerate}
...
\begin{align}
    ...
    I &: (x, y) \mapsto (x, y).
\end{align}
At each layer transition $s \to s+1$, the three available moves are exactly $X$, $Y$, and $I$, and a Hamiltonian decomposition corresponds to choosing, at each $(x, y, s)$, which of $\{X, Y, I\}$ is assigned to which cycle---an ordered 1-factorization of the bipartite graph on each layer whose vertex classes are the two copies of $\Zm^2$ (at layers $s$ and $s+1$) and whose three perfect matchings are $I$, $X$, and~$Y$.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/round_map_cycle.pdf}
\caption{Round maps for the odd case ($m = 5$, top) and even case ($m = 6$, bottom). Each panel shows the single $m^2$-cycle traced by one color's round map on $\ZZ_m^2$. Arrows darken along the traversal order; the open circle marks the start vertex $(0,0)$. All six round maps are verified to be single cycles.}
\label{fig:round-map}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/diagonal_gadget.pdf}
\caption{The diagonal gadget on $\ZZ_7^2$. Matching~$A$ uses $X$ (blue, vertical) off the diagonal $D = \{(x,y) : x+y \equiv 0\}$ and $Y$ (red, horizontal) on~$D$. Matching~$B$ reverses the roles. Both are Hamiltonian cycles on $\ZZ_m^2$; the entire odd-case construction is built from words in $A$ and~$B$.}
\label{fig:diagonal-gadget}
\end{figure}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Knuth (2026)}~\cite{knuth2026}: Odd-case construction via Claude Opus~4.6. Exactly 760 ``Claude-like'' decompositions valid for all odd $m > 1$. Even case left open. The March~6 revision acknowledges the present work's independent odd-case construction and even-case solution.
\end{itemize}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/layer_stack.pdf}
\caption{Layer structure of the even-case construction for $m = 12$. The first $m{-}2 = 10$ layers are constant $XYI$ everywhere (blue). The penultimate staircase layer ($s = 10$) contains the diagonal walk and parity-dependent structure. The terminal layer ($s = 11$) is a row-block of $YIX$/$IYX$.}
...
\begin{itemize}
    \item \textbf{Five explorations, no human intervention.} The same milestone (odd case solved for all $m$) that Knuth's Claude reached in 31~explorations with significant coaching.\footnote{Exploration count is an imperfect metric. Many of Claude's 31~explorations involved reasonable strategies---testing linear and quadratic functions, trying serpentine constructions---that happen to fail on this particular problem but might have succeeded on a different one. As Knuth notes~\cite{knuth-pc}, optimizing for any particular metric is often misleading. The comparison is reported as a factual observation, not a claim that fewer explorations is unconditionally better.}
...
The orchestrator agent (a third Claude Opus~4.6 Thinking instance, directed by the author) made five key decisions, each documented in the meta-observation log (\Cref{app:meta}).
...
\begin{enumerate}[label=(\alph*)]
    \item \textbf{Compression:} 5~explorations vs.\ 31 for the same milestone (odd case solved), though exploration count is a crude proxy (see footnote in \Cref{sec:fast}). The structured logging forced each exploration to be maximally informative.
...
\begin{enumerate}[label=(\alph*)]
    ...
    \item The even case required \emph{cross-agent synthesis}: structure from one model, data from another, integration by an orchestrator. This is not the generate-and-check loop of single-agent theorem proving. It is something more collaborative.
\end{enumerate}
...
\begin{enumerate}[label=(\alph*)]
    ...
    \item The even-case staircase construction, while verified to $m = 2{,}000$ (and spot-checked to $m = 30{,}000$), lacks a symbolic proof. Whether this can be proved by the same agents (or any current AI system) is an open question.
    \item Two natural symmetries generate additional decompositions from any given one. First, one may \emph{conjugate} the layer indexing: reversing the roles of layers $0$ and $m{-}1$ (so that the terminal layer sits at $s = 0$ and the staircase at $s = m{-}1$, or any cyclic shift) yields a valid decomposition whenever the original is, since the round-map criterion depends only on the cyclic product of all $m$~layers. Second, \emph{reversing} a decomposition---traversing each Hamiltonian cycle backwards and reading directions after each step rather than before---gives another valid decomposition. These symmetries, noted by Knuth~\cite{knuth-pc}, enlarge the family of known decompositions but do not change the underlying construction.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{The even-case proof is computational.} The construction of \Cref{def:even} is verified for all even $m \leq 2{,}000$ (with spot checks to $m = 30{,}000$), but a symbolic proof of correctness for all even $m$ remains open. The staircase layer's parity-sensitive case structure makes direct symbolic analysis substantially harder than the odd case.
    ...
    \item \textbf{The orchestration was reconstructed.} The orchestrator agent's decisions were documented in a meta-observation log written during and after the sessions, not in strict real-time. Future experiments should log orchestration decisions as they happen.
...
\begin{itemize}
    ...
    \item For even $m \geq 4$: closed-form construction via bulk layers plus staircase and terminal layers (\Cref{thm:even}), verified computationally for all $m \leq 2{,}000$, with spot checks to $m = 30{,}000$.
\end{itemize}

\textbf{The methodological result.} A structured exploration prompt (Residue) reduces the path to the odd-case milestone from 31~explorations to~5 (though exploration count is an imperfect metric---see \Cref{sec:fast}), while enabling graceful failure documentation and cross-agent data transfer. Two agents with different reasoning styles---one top-down symbolic, one bottom-up computational---produce complementary artifacts that, when combined by an orchestrator agent (also Claude Opus~4.6 Thinking), solve a problem neither could solve alone.
...
\newblock Stanford Computer Science Department, February 28, 2026; revised March 6, 2026.
...
\newblock Arc-disjoint {H}amiltonian paths in {C}artesian products of directed cycles.
\newblock {\em Ars Mathematica Contemporanea}, 25(2), 2025.
\newblock Preprint: \texttt{arXiv:2203.11017} (2022).
...
\bibitem{morrison-lean}
K.~Morrison.
\newblock Lean~4 formalization of Knuth's odd-case proof. Posted online March~4, 2026; described in Knuth~\cite{knuth2026}, postscript.

\bibitem{reitbauer2026}
M.~Reitbauer.
\newblock Simplified odd-case construction via {GPT}~5.4 and {C}laude~4.6 {S}onnet. Described in Knuth~\cite{knuth2026}, postscript (March 2026).

\bibitem{ho-proof}
H.~B. Suan.
\newblock Proof of correctness for~\cite{knuth-even-closed}, generated by {GPT}-5.4~{P}ro. Described in Knuth~\cite{knuth2026}, postscript (March 2026).

\bibitem{knuth-pc}
D.~E. Knuth.
\newblock Personal communication, March 2026.
