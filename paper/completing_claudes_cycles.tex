...
In the postscript to the revised paper (March~4, further revised March~6), Knuth reported a rapid sequence of developments: Stappers put Claude Opus~4.6 to work on the even case, producing a search-based solution via ORTools CP-SAT~\cite{knuth-even-solution}; Ho Boon Suan produced a computational construction for even $m \geq 8$ using GPT-5.3-codex~\cite{knuth-even-closed}, describing the pattern as ``far more chaotic'' than the odd case; Kim Morrison formalized Knuth's odd-case proof in Lean~4~\cite{morrison-lean}; and Exocija found what Knuth called ``probably the simplest possible'' odd-case construction by interacting with GPT~5.4 and Claude~4.6 Sonnet~\cite{reitbauer2026}. Ho subsequently used GPT-5.4~Pro to produce a complete 14-page proof of~\cite{knuth-even-closed}'s correctness without human editing~\cite{ho-proof}. Finally, Knuth acknowledged the present work, describing the even-case construction as ``considerably simpler'' than~\cite{knuth-even-closed} and noting that the process analysis has ``potentially significant implications for how new problems can be tackled and resolved in the future.''
