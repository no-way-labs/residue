...
The construction was verified computationally for every odd $m$ from 3 to 199. The parameter choices observed were:
\begin{itemize}
...
\begin{enumerate}[label=(\alph*)]
    ...
    \item The even-case staircase construction, while verified to $m = 200$ (and spot-checked to $m = 500$), lacks a symbolic proof. Whether this can be proved by the same agents (or any current AI system) is an open question.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{The even-case proof is computational.} The construction of \Cref{def:even} is verified for all even $m \leq 200$ (with spot checks to $m = 500$), but a symbolic proof of correctness for all even $m$ remains open. The staircase layer's parity-sensitive case structure makes direct symbolic analysis substantially harder than the odd case.
...
\begin{itemize}
    ...
    \item For even $m \geq 4$: closed-form construction via bulk layers plus staircase and terminal layers (\Cref{thm:even}), verified computationally for all $m \leq 200$, with spot checks to $m = 500$.
\end{itemize}
