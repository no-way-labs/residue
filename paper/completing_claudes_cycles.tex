...
The constructions are produced by two LLM agents operating under a shared structured exploration prompt (``Residue''), with a human orchestrator routing data and tools between them. The same prompt applied to different models produces radically different strategies: one agent reasons top-down and produces symbolic proofs; the other reasons bottom-up and builds solvers. Neither solves the even case alone. The combination does. The three exploration logs---two agent-level, one orchestrator-level---constitute a controlled case study in LLM mathematical reasoning, with implications for prompt design, multi-agent coordination, and the nature of the conception bottleneck in automated theorem proving.
...
\begin{enumerate}[label=(\roman*)]
    ...
    \item \textbf{Methodological.} We document how these constructions were produced by two LLM agents operating under a shared structured exploration prompt, with a human orchestrator mediating between them. The same prompt applied to different model providers produced radically different mathematical strategies. The full exploration logs are included as appendices.
\end{enumerate}
...
% ============================================================
\section{The Fast Agent's Trajectory}
\label{sec:fast}

The fast agent (Provider~A) is a top-down symbolic reasoner. Given the Residue prompt and the problem statement, it proceeded as follows.

\begin{table}[h]
\centering
\caption{Fast agent exploration summary.}
\label{tab:fast}
...
\begin{itemize}
    ...
    \item \textbf{Different construction.} The fast agent's layer-count approach is algebraically cleaner than Knuth's vertex-level rule, but produces a different (smaller) family.
    ...
    \item \textbf{The serpentine attractor was skipped entirely.} Knuth's Claude spent $\sim$10 explorations on serpentine constructions. The fast agent never considered them---it went directly to the symbolic diagonal family.
\end{itemize}

\subsection{Even-Case Push (Explorations 6--25)}

After the odd case was solved, the fast agent was directed to the even case with a 15-exploration budget. Key milestones:
...
\begin{itemize}
    ...
    \item \textbf{Exploration~16:} Received fiber-coordinate tables from the slow agent (see \Cref{sec:orchestration}). Immediate pattern recognition: $m-2$ bulk $XYI$ layers plus two structured repair layers.
    \item \textbf{Exploration~21--22:} Used the seeded solver (adapted from the slow agent's MRV solver) to obtain exact solutions for $m = 14$ and $m = 16$. Refitted the staircase rule.
...
% ============================================================
\section{The Slow Agent's Trajectory}
\label{sec:slow}

The slow agent (Provider~B) is a bottom-up computational solver. Given the identical Residue prompt and problem statement, it followed a markedly different trajectory.

\begin{table}[h]
\centering
\caption{Slow agent exploration summary.}
\label{tab:slow}
\begin{tabular}{@{}clp{8cm}@{}}
\toprule
\# & Outcome & Action \\
...
\begin{itemize}
    ...
    \item \textbf{More even-case data than any other single agent.} Solutions for $m = 4, 6, 8, 10, 12$---the raw material that later unblocked the fast agent's even-case push.
    \item \textbf{Never found the fiber-coordinate framework.} The slow agent noted the fiber/coset structure (\Cref{tab:slow}, exploration~7) but did not build on it. The Reformulations field can preserve an insight but cannot force a model to recognize which insight is load-bearing.
\end{itemize}
...
\subsection{Decision 2: Directing the Fast Agent to Even}

After the fast agent solved the odd case, direct it to the even case with a 15-exploration budget. The slow agent was still running independently.

\textbf{Outcome:} The fast agent discovered the layer-sign parity invariant (a new structural result) and found $m = 6, 8$, but stalled at $m = 10$.

\subsection{Decision 3: Cross-Agent Data Transfer}

The slow agent had solutions for $m = 10$ and $m = 12$. The fast agent had the structural framework but could not reach $m = 10$. The orchestrator asked the slow agent to export its solutions in fiber-coordinate format (the fast agent's native representation) and passed the resulting tables to the fast agent.

\textbf{Outcome:} Immediate pattern recognition. The fast agent identified the $m-2$ bulk layers plus two repair layers within minutes of receiving the data. This was the critical move that neither agent could have made alone.

\subsection{Decision 4: Tool Transfer}

The fast agent needed to verify predictions at $m = 14, 16$ but could not compute exact solutions. The slow agent had the MRV solver. The orchestrator extracted the solver and placed it in the fast agent's working directory.

\textbf{Outcome:} The fast agent did not use the solver as given---it adapted it into a \emph{seeded} solver, using its own structural predictions to constrain the search domain. The seeded solver is faster than either the raw MRV or the fast agent's analytical approach alone.

\subsection{Decision 5: Context About External Results}

The orchestrator informed the fast agent that other researchers (Stappers with Claude, Ho Boon Suan with GPT-5.3-codex) had found working even-case constructions but described them as ``chaotic'' and ``far more complex'' than the odd case. No constructions were provided.

\textbf{Outcome:} This provided motivation and calibration. The fast agent's construction turned out to be simpler than both external solutions.

\subsection{What the Orchestrator Did That the Prompt Could Not}

\begin{enumerate}
    \item \textbf{Recognized complementary strengths.} The fast agent had theory; the slow agent had data. Neither knew the other existed.
    \item \textbf{Chose the transfer representation.} Fiber coordinates were the fast agent's native language. Converting the slow agent's output to fiber tables made the transfer immediate rather than requiring the fast agent to parse a foreign format.
    \item \textbf{Timed the transfers.} Data was sent after the fast agent had stalled (not before, which would have been noise), and the tool was sent after the fast agent had demonstrated it could use the data but lacked verification capacity.
\end{enumerate}
...
\begin{enumerate}[label=(\alph*)]
    ...
    \item \textbf{Bounded dead ends:} The slow agent spent 5~explorations on serpentines (vs.\ $\sim$10 for Knuth's Claude). The ``What This Rules Out'' field generalized failures to approach classes, preventing minor-variation grinding.
    \item \textbf{Graceful escalation:} The fast agent's $m = 10$ stall produced a useful handoff specification, not a degradation. The structured failure record (specific basin, specific diagnosis, specific gap) made the cross-agent transfer actionable.
...
\begin{enumerate}[label=(\alph*)]
    ...
    \item \textbf{Tool transfer as amplification.} The seeded solver was better than either the raw MRV or the analytical approach alone. The fast agent improved the slow agent's tool by combining it with its own structural knowledge.
...
\begin{enumerate}[label=(\alph*)]
    \item The fast agent's odd-case solution was an \emph{interpolation}: it recombined known techniques (fiber coordinates, skew maps, gcd criteria) in a novel arrangement. This is creative but within the space of existing mathematical tools.
    \item The slow agent's MRV breakthrough was an \emph{algorithmic} creative act: designing a better search algorithm. This doesn't fit neatly into ``conception'' vs.\ ``search.''
...
\begin{itemize}
    \item \textbf{Fast agent:} 28 explorations (5 for odd case, 23 for even case including Lean formalization attempts). Total: approximately 65{,}000 words.
    \item \textbf{Slow agent:} 15 explorations. Total: approximately 25{,}000 words.
\end{itemize}
